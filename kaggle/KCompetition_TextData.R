...
# Then create a corpus from the description variable. You can use other variables
# in the dataset for text analytics, but we will just show you how to use this particular variable. 
...
# The head function takes the first "n" rows of DescriptionWords 
# (the first argument to the head function), where "n" is specified by the second argument to the head function. 
# So here we are taking the first nrow(eBayTrain) observations from DescriptionWords,
# and putting them in a new data frame called "DescriptionWordsTrain"

DescriptionWordsTrain = head(DescriptionWords, nrow(eBayTrain))
str(DescriptionWordsTrain)
str(eBayTrain)
# The tail function takes the last "n" rows of DescriptionWords 
# (the first argument to the tail function), where "n" is specified by the second argument to the tail function. 
# So here we are taking the last nrow(eBayTest) observations from DescriptionWords,
# and putting them in a new data frame called "DescriptionWordsTest"

DescriptionWordsTest = tail(DescriptionWords, nrow(eBayTest))

# Note that this split of DescriptionWords works to properly put the observations 
# back into the training and testing sets, because of how we combined them together when we first made our corpus.

# Before building models, we want to add back the original variables from our datasets.
# We'll add back the dependent variable to the training set, and the WordCount 
# variable to both datasets. You might want to add back more variables to use in your model - we'll leave this up to you!
